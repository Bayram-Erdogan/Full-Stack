...
const { describe, test } = require('node:test');    /*  ==> Test yazmak için kullanılan bir modüldür. describe ve test
...
/*
    Daha sonra test grubumu taniliyorum ve icerisine const listWithOneBlog (Tek bir blog nesnesi içeren bir dizi. Blog
    nesnesi,blogun ID'sini, başlığını, yazarını, URL'sini, beğeni sayısını ve versiyon numarasını içerir.) icinde örnek
    veriyi ekledim.
*/
...
/*
    Özetle, bu test listHelper.totalLikes fonksiyonunun doğru çalıştığını ve tek bir blog içeren bir listede toplam
    beğeni sayısını doğru hesapladığını doğrulamak için yazılmıştır.
*/
  })
